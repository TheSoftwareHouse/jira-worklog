...
const findWorklogs = async (client, project, username, day) => client
  .search
  .search({
    jql: `project = ${project} AND worklogAuthor = currentUser() AND worklogDate = ${day} ORDER BY updated ASC`,
    fields: ['worklog'],
  })
  .then(res => res.issues.map((task) => {
    const spentSeconds = task.fields.worklog
      .worklogs
      .filter(
        worklog => (worklog.author.emailAddress === username || worklog.author.name === username)
...
module.exports = {
  checkCredentials: async (host, account, password) => {
    try {
      const client = new JiraClient({
        host,
        basic_auth: { username: account, password },
      });
  ...
  initialize: (host, account, password) => {
    ...
    return {
      getWorklogs: (project, day) => findWorklogs(client, project, username, day),

      getSuggestedTaskKeys: (project, day) => client
        .search
      ...
      findTasksWithKeys: (keys) => {
        ...
        return client
          ...
          .then(result => result.issues.map(
            task => ({ key: task.key, name: task.fields.summary })
          ));
      },

      sendWorklog: (day, task, hours) => client
        .issue
        .addWorkLog({
          issueKey: task,
          notifyUsers: false,
