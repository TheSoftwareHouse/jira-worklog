...
module.exports = {
  ...
  promptDay: async () => inquirer
    .prompt({
      ...
      choices: Array
        ...
        .map(date => ({
          name: chalk.cyan(date.format('YYYY-MM-DD')) + chalk.dim(date.format(' (dddd)')),
  ...
  promptHours: async (choices, defaultChoice) => inquirer
    .prompt({
      type: 'list',
      name: 'hours',
  ...
  promptConfirmation: async (hours, task) => inquirer
    .prompt({
      type: 'confirm',
      name: 'confirmed',
