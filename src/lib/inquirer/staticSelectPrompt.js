...
class StaticSelectPrompt extends Base {
    constructor(questions, rl, answers) {
        ...
        if (initialValue) {
            let index = _.findIndex(this.opt.choices.realChoices, ({value}) => value === initialValue);
    ...
    _run(cb) {
        ...
        events.line.pipe(take(1), map(this.getCurrentValue.bind(this)), flatMap(value => runAsync(self.opt.filter)(value).catch(err => err)))
    ...
    onDownKey() {
        this.selected = this.selected < this.opt.choices.realLength - 1 ? this.selected + 1 : this.selected;
        this.render();
    }
}
